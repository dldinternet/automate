...
/*
Returns a net.Conn struct suitable for setting up a streaming server connection
...
*/
func reportingServerConnection() (net.Conn, err) {
	...
	return grpc.DialContext(ctx, "bufnet", grpc.WithDialer(dialer), grpc.WithInsecure())
}

func TestReportingServerExport(t *testing.T) {
	conn, err := reportingServerConnection()
	defer conn.Close()
	require.NoError(err)
